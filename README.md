...
这里是一套覆盖C语言从入门、数据结构、图形与动画编程、图像处理、网络编程、数值计算到AI编程与大语言模型训练推理的（无python依赖）的代码集成。
...
初学C语言，一般学校老师会推荐 Dev C++等；这个是误人子弟！如果不是从事计算机或信息行业的，Dev C++无妨。天天用乐高堆砌飞机，终究是无法飞行的！



如果未来将C语言作为重要能力或者学习其他语言，强烈推荐 **Visual Studio** Community。VS 是宇宙之内最好的IDE，没有之一。全世界真正的程序员都选择它。
